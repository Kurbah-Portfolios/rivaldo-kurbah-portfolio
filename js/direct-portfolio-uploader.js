(() => {
  ...
  const styles = `
    .rk-upload-overlay{position:fixed;inset:0;background:rgba(2,6,23,.86);z-index:9999;display:flex;align-items:center;justify-content:center;padding:20px;backdrop-filter:blur(5px)}
    .rk-upload-modal{width:min(720px,100%);max-height:92vh;overflow:auto;background:#0f172a;border:1px solid #334155;border-radius:18px;box-shadow:0 25px 70px rgba(0,0,0,.45);padding:26px;color:#f8fafc;font-family:inherit}
    ...
    .rk-upload-filemeta{font-size:.82rem;color:#94a3b8;margin:-8px 0 18px;word-break:break-word}
    ...
    .rk-token-note{padding:13px 14px;border:1px solid #334155;border-radius:10px;background:#0b1220;color:#94a3b8;font-size:.8rem;line-height:1.55;margin-bottom:16px}.rk-token-note strong{color:#facc15}.rk-token-note a{color:#38bdf8}
  ...
  async function verifyOwner(token) {
    const user = await apiFetch('https://api.github.com/user', token);
    if (!user || String(user.login || '').toLowerCase() !== OWNER_LOGIN) {
      throw new Error('This upload token must belong to the KurbahTech GitHub account.');
    }
  }
  ...
  async function putContent(path, content, token, message, sha) {
    ...
    if (sha) body.sha = sha;
    return apiFetch(`${API}/contents/${path}`, token, {
      method: 'PUT',
  ...
  async function deleteContent(path, sha, token, message) {
    if (!sha) return;
    try {
      await apiFetch(`${API}/contents/${path}`, token, {
        method: 'DELETE',
        headers: { 'Content-Type': 'application/json' },
  ...
  function setStatus(el, type, text) {
    el.className = `rk-upload-status ${type}`;
    el.textContent = text;
  }
  ...
  function buildModal(mode, file, config) {
    ...
    overlay.className = 'rk-upload-overlay';
    overlay.setAttribute('role', 'dialog');
    overlay.setAttribute('aria-modal', 'true');

    const modal = document.createElement('div');
    modal.className = 'rk-upload-modal';
    const head = document.createElement('div');
    head.className = 'rk-upload-head';
    const title = document.createElement('h2');
    title.textContent = mode === 'certificate' ? 'Upload Certificate' : 'Upload Faith & Ministry Item';
    const close = document.createElement('button');
    close.type = 'button';
    close.className = 'rk-upload-close';
    close.setAttribute('aria-label', 'Close uploader');
    close.textContent = '×';
    head.append(title, close);
    modal.appendChild(head);

    const preview = document.createElement('div');
    preview.className = 'rk-upload-preview';
    ...
    grid.appendChild(field('GitHub upload token', 'token', 'input', {
      full: true,
      required: true,
      inputType: 'password',
      placeholder: 'Fine-grained token for this repository only',
      autocomplete: 'off'
    }));
    form.appendChild(grid);

    const note = document.createElement('div');
    note.className = 'rk-token-note';
    note.innerHTML = '<strong>Security:</strong> use a fine-grained GitHub personal access token restricted to <b>Kurbah-Portfolios/rivaldo-kurbah-portfolio</b> with <b>Contents: Read and write</b>. The token is used only for this upload and is not saved by the page. <a href="https://github.com/settings/personal-access-tokens" target="_blank" rel="noopener">Manage fine-grained tokens</a>.';
    form.appendChild(note);

    const actions = document.createElement('div');
    actions.className = 'rk-upload-actions';
    const submit = document.createElement('button');
    submit.type = 'submit';
    submit.className = 'rk-upload-submit';
    submit.textContent = 'Upload & Publish';
    const cancel = document.createElement('button');
    cancel.type = 'button';
    cancel.className = 'rk-upload-cancel';
    cancel.textContent = 'Cancel';
    actions.append(submit, cancel);
    form.appendChild(actions);
    const status = document.createElement('div');
    status.className = 'rk-upload-status';
    ...
    form.addEventListener('submit', async e => {
      ...
      setStatus(status, 'info', 'Verifying owner account and preparing upload…');
      ...
      try {
        ...
        if (!ext) throw new Error('Unsupported image format. Use JPG, PNG, WEBP or GIF.');
        const nameSeed = mode === 'certificate' ? values.heading : values.title;
        const timestamp = new Date().toISOString().replace(/[-:.TZ]/g, '').slice(0, 14);
        const folder = mode === 'certificate' ? 'images/certificates/uploads' : 'images/faith-ministry/uploads';
        uploadedPath = `${folder}/${timestamp}-${slugify(nameSeed)}.${ext}`;
        const base64 = await fileToBase64(file);

        setStatus(status, 'info', 'Uploading image to the portfolio repository…');
        const imageResult = await putContent(uploadedPath, base64, token, `Upload ${mode} image: ${nameSeed}`);
        uploadedSha = imageResult && imageResult.content && imageResult.content.sha;

        const dataPath = mode === 'certificate' ? 'data/certificates-managed.json' : 'data/faith-managed.json';
        const managed = await getManagedData(dataPath, token);
        const addedAt = new Date().toISOString();
        const record = mode === 'certificate'
          ? {
              id: `certificate-${Date.now()}`,
              heading: String(values.heading || '').trim(),
              program: String(values.program || '').trim(),
              description: String(values.description || '').trim(),
              image: uploadedPath,
              verify_url: String(values.verify_url || '').trim(),
              learn_url: String(values.learn_url || '').trim(),
          ...
          : {
              ...
              link_label: String(values.link_label || '').trim(),
              added_at: addedAt
            };
        managed.data.unshift(record);
        const jsonBase64 = encodeBase64Utf8(JSON.stringify(managed.data, null, 2) + '\n');

        setStatus(status, 'info', 'Publishing metadata…');
        await putContent(dataPath, jsonBase64, token, `Publish ${mode}: ${nameSeed}`, managed.sha);

        form.elements.token.value = '';
        setStatus(status, 'success', 'Published successfully. GitHub Pages may take a short time to refresh.');
        submit.textContent = 'Published';
        if (typeof config.onSuccess === 'function') config.onSuccess(record);
      } catch (err) {
        if (uploadedPath && uploadedSha) {
          await deleteContent(uploadedPath, uploadedSha, token, `Rollback failed ${mode} upload`);
        }
  ...
  function openPicker(mode, config) {
    ...
    input.addEventListener('change', () => {
      ...
      if (file.size > MAX_FILE_BYTES) {
        alert('The selected image is larger than 8 MB. Please use a smaller image.');
        return;
      }
      if (!extensionFor(file)) {
        alert('Unsupported image format. Please use JPG, PNG, WEBP or GIF.');
        return;
      }
      buildModal(mode, file, config);
    }, { once: true });
    input.click();
  }
